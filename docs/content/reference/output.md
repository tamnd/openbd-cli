...
describes what users get. Pick a format with `-o`, or let openbd choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openbd <command> -o table   # aligned columns for reading
openbd <command> -o jsonl   # one JSON object per line, for piping
openbd <command> -o json    # a single JSON array
openbd <command> -o csv     # spreadsheet friendly
openbd <command> -o tsv     # tab-separated
openbd <command> -o url     # just the URL column
openbd <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openbd <command> --fields id,title,url
```
...
```bash
openbd <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openbd <command>            # a table, because this is a terminal
openbd <command> | wc -l    # JSONL, because this is a pipe
```
