// Package cli assembles the openbd command tree from the openbd
...
// NewApp assembles the kit application from the openbd domain. The
...
//
// To add a command, declare it in openbd/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openbd.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openbd.Domain{}).Register(app)
